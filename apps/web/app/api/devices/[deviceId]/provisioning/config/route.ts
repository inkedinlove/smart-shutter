...
function buildFileName(deviceId: string): string {
  return `${deviceId}-config.h`;
}
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    console.error("Unable to generate provisioning config.");
    return apiError("Unable to generate provisioning config.", 500);
  }
}
